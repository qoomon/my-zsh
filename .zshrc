...
function zconfig::profile { 
  (export ZCONFIG_PROFILE='yes'; zsh -ic exit)
}

function zconfig::profile::before { 
...
function zconfig::profile::after { 
  zprof
  unsetopt xtrace
  exec 2>&3 3>&-
...
[ "${ZCONFIG_PROFILE:-'no'}" = 'yes' ] && zconfig::profile::before
zconfig::load
[ "${ZCONFIG_PROFILE:-'no'}" = 'yes' ] && zconfig::profile::after
