...
# colorized log
git config --global alias.graph $'log --color=always --graph --all --date=format:\'%a %Y-%m-%d %H:%M\' --pretty=format:\' %C(blue bold)%h%C(reset) %C(white bold)%s%C(reset) %C(dim white)%an%C(reset)%n ↪  %C(dim green)%ar%C(reset) %C(dim cyan)%ad%C(reset)%C(auto)%d%C(reset)%n\' -m'
